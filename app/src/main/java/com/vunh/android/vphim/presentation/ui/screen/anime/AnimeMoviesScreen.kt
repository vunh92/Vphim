...
@Composable
fun AnimeMoviesScreen(
    viewModel: AnimeMoviesViewModel = hiltViewModel()
) {
    ...
    LaunchedEffect(shouldLoadMore) {
        if (shouldLoadMore && !uiState.isLoadingMore && uiState.hasMorePages) {
            viewModel.onEvent(AnimeMoviesUiEvent.LoadMore)
        }
    }
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = {
                    Text(
                        text = stringResource(R.string.home_anime_movies_title),
        ...
        PullToRefreshBox(
            ...
            onRefresh = { viewModel.onEvent(AnimeMoviesUiEvent.Refresh) },
            modifier = Modifier
                .fillMaxSize()
