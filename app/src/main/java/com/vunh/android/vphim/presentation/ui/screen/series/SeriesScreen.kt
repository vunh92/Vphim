...
fun SeriesScreen(
    ...
    LaunchedEffect(shouldLoadMore) {
        if (shouldLoadMore && !uiState.isLoadingMore && uiState.hasMorePages) {
            viewModel.onEvent(SeriesUiEvent.LoadMore)
        }
    }
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = {
                    Text(
                        text = stringResource(R.string.home_series_movies_title),
        ...
        PullToRefreshBox(
            ...
            onRefresh = { viewModel.onEvent(SeriesUiEvent.Refresh) },
            modifier = Modifier
                .fillMaxSize()
